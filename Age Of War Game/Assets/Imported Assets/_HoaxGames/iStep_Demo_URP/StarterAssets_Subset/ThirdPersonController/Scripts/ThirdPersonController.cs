...
namespace StarterAssets
{
    ...
    public class ThirdPersonController : MonoBehaviour
    {
        ...
        private void improvedGroundedCheckByiStep()
        {
            ...
            {
                targetSlopeVelocity = Vector3.Project(-transform.up, targetSlopeVelocity);
                m_slopeVelocity += targetSlopeVelocity * Time.deltaTime * Mathf.Abs(Gravity) * 3;
            }
            else
            {
        ...
        private void Move()
        {
            ...
            {
                Vector3 slopeVelocityToUse = calculateSlopeVelocityToUseByiStep(targetDirection, targetSpeed); //iStep improvement
        ...
        private Vector3 calculateSlopeVelocityToUseByiStep(Vector3 targetDirection, float targetSpeed)
        {
            // Function Copyright © Kreshnik Halili

            if (m_slopeVelocity.magnitude > 0.001f && targetSpeed > 0.001f)
            {
                Vector3 verticalVel = Vector3.Project(m_slopeVelocity, -transform.up);
                Vector3 slopeVelPlanar = m_slopeVelocity - verticalVel;
                float dot = Vector3.Dot(slopeVelPlanar.normalized, targetDirection.normalized);

                if (dot > 0)
                {
                    Vector3 subtract = Vector3.Project(targetDirection.normalized * targetSpeed, slopeVelPlanar.normalized);
                    Vector3 nextSlopeVelPlanar = slopeVelPlanar - subtract;

                    dot = Vector3.Dot(nextSlopeVelPlanar.normalized, slopeVelPlanar.normalized);

                    if (dot > 0) return nextSlopeVelPlanar + verticalVel;
